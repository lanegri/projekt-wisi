...
from app.Aufgabe3.Aufgabe_3_1.transform_system_of_equations import (
    q_r_with_householder_transformation, q_r_with_givens_rotation)


def solve_least_squares_householder(A, b):
    Q, R = q_r_with_householder_transformation(A)
    Q_T_b = Q.T @ b
    n = A.shape[1]
    b1 = Q_T_b[:n]
...
def solve_least_squares_givens(A, b):
    Q, R = q_r_with_givens_rotation(A)
    Q_T_b = Q.T @ b
    n = A.shape[1]
    b1 = Q_T_b[:n]
